...
const CertIssueIndividual = ({ category, setCategory, }) => {
  ...
  if (isLoading) return <LoadingPage status={status} category={category} setCategory={setCategory} setIsLoading={setIsLoading} setWalletaddress={setWalletaddress} setBatchDescription={setBatchDescription} selectImage={selectImage} setUploadedFileName={setUploadedFileName}/>;
...
const LoadingPage = ({ status, category,setCategory,setIsLoading,setWalletaddress,setBatchDescription,selectImage,setUploadedFileName}) => {
  const { t } = useTranslation();
